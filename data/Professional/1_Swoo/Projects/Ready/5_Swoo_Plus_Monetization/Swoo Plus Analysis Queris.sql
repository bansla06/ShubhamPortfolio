...
FROM `premium_games_db.ua_pro_game_processed_db_v1` 
...
LEFT JOIN (
SELECT ua_notification_token, user_id
FROM `master_tables.user_device` ) b
ON a.device_channel = b.ua_notification_token 
INNER JOIN(
SELECT user_id, sum(amount) AS amt
FROM `premium_games_db.ripple_user_successful_txn` 
GROUP BY 1) c
...
FROM `your-project-id.premium_games_db.ripple_packages`) B
...
FROM `your-project-id.premium_games_db.ripple_user_successful_txn` ) b
...
FROM(
SELECT date , device_channel
FROM  `premium_games_db.ua_pro_app_processed_db_v1` 
