SELECT event_name FROM `app_analytics.bingo_clicks` 
...
table_truncate(dau_wau_mau,'DAU_WAU_MAU','processed_db')
...
from app_analytics.urban_airship
...
SELECT REPLACE(JSON_EXTRACT(body, "$['properties'].gameId"), "\"", "") as game_id,body_name,MIN(occurred) as min_occ FROM `app_analytics.urban_airship` 
...
SELECT body_name FROM `app_analytics.urban_airship` 
WHERE occurred BETWEEN TIMESTAMP('2018-09-01') AND TIMESTAMP('2018-09-24') GROUP BY 1


SELECT body_name,occurred,COUNT(*) FROM `app_analytics.urban_airship` 
...
SELECT DATE(occurred) as date,device_channel FROM `app_analytics.urban_airship` 
...
SELECT DATE(occurred) as date,COUNT(DISTINCT device_channel) as Users_who_got_ringer FROM `app_analytics.urban_airship` 
WHERE occurred BETWEEN TIMESTAMP('2018-09-01') AND TIMESTAMP('2018-09-25') 
AND body_name IN ('triviaringer_open','bingoringer_open')
GROUP BY 1
...
SELECT DATE(occurred) as date,COUNT(DISTINCT device_channel) as Users_who_dismissed_ringer FROM `app_analytics.urban_airship` 
WHERE occurred BETWEEN TIMESTAMP('2018-09-01') AND TIMESTAMP('2018-09-25') 
AND body_name IN ('triviaringer_dismiss','bingoringer_dismiss')
GROUP BY 1
...
SELECT DATE(occurred) as date,device_device_type as OS,COUNT(*) as Users_who_clicked_ringer_back FROM `app_analytics.urban_airship` 
WHERE occurred BETWEEN TIMESTAMP('2018-09-01') AND TIMESTAMP('2018-09-25') 
...
FROM `your-project-id.app_analytics.user_opened_watched` 
...
SELECT COUNT(DISTINCT device_channel) FROM app_analytics.urban_airship
WHERE occurred BETWEEN TIMESTAMP('2018-09-23') AND TIMESTAMP('2018-09-24')
AND body_name IN ('trivia_signal_received','bingo_signal_received') --,'_signal_received')

159760

SELECT DATE(occurred) as Date,COUNT(DISTINCT device_channel) as Users FROM app_analytics.urban_airship
...
from `app_analytics.branch_data` where dATE(timestamp)>='2018-07-01' AND date(timestamp) <= '2018-09-24'
...
select date(timestamp) as date, developer_identity
from `app_analytics.branch_data` where dATE(timestamp)>='2018-09-01' AND date(timestamp) <= '2018-09-24'
and name='BINGO_STARTED_PLAYING'
...
FROM `your-project-id.app_analytics.ua_candy_crush` 
...
SELECT DATE(timestamp) as Date, COUNT(DISTINCT developer_identity)as Users FROM `your-project-id.app_analytics.branch_io`
...
FROM `your-project-id.app_analytics.branch_io`
where DATE(timestamp) >= '2018-05-01'
...
FROM `your-project-id.app_analytics.urban_airship` 
...
LEFT JOIN (
SELECT CAST(id AS STRING) as id,email,full_name,phone  FROM `your-project-id.app_analytics.user` 
GROUP BY 1,2,3,4) b
...
FROM `your-project-id.app_analytics.branch_io` bi1
JOIN `your-project-id.app_analytics.branch_io` bi2
...
SELECT device_channel,game_id,occurred FROM `your-project-id.processed_db.fullevents_for_users_containing_candy` 
...
ON a.developer_identity = b.developer_identity
LEFT JOIN (
SELECT developer_identity,date FROM `processed_db.bingo_wau_cohort_temp_table` 
GROUP BY 1,2) c
...
FROM `your-project-id.app_analytics.urban_airship_v2`
...
FROM
`your-project-id.app_analytics.branch_io` bi1
RIGHT JOIN
`your-project-id.app_analytics.branch_io` bi2
ON
...
FROM `app_analytics.user_sessions`
...
AND name = "BINGO_STARTED_PLAYING"
GROUP BY 1,2) a
CROSS JOIN `your-project-id.app_analytics.dates_refer` b
...
FROM `your-project-id.app_analytics.branch_io` ds
WHERE Date(timestamp)>='2018-10-10'
GROUP BY 1) a
JOIN (
...
FROM `gaming_service_db`.`user_claim`
WHERE status = "1" 
...
FROM `app_analytics.urban_airship_processed_db_v1`
...
FROM `processed_db.landing_page_temp_date_nov21`
...
JOIN (
SELECT *
FROM `processed_db.ua_app_version_details`) b
ON a.device_channel = b.device_channel
GROUP BY 1,2,3,4
...
FROM `processed_db.ua_landing_page_comparision` 
...
FROM `processed_db.ua_landing_page_comparision_v3` 
WHERE type = 'OPEN'
AND app_version IS NOT NULL 
...
FROM `processed_db.ua_app_version_details` 
GROUP BY 1)
...
FROM `your-project-id.app_analytics.urban_airship_v2`  ua
JOIN `your-project-id.gaming_service_db.game` g
...
FROM `your-project-id.gaming_service_db.trivia_question_set` tqs 
JOIN `your-project-id.gaming_service_db.game` g
on g.game_id = tqs.game_id
JOIN `your-project-id.gaming_service_db.user_game_statistics` ugs
...
LEFT JOIN (
SELECT date,CASE WHEN app_version >= '6.4.0' THEN 'new' ELSE 'old' END AS landpagetype,COUNT(DISTINCT device_channel) as users
FROM `processed_db.ua_landing_page_comparision_v2` 
WHERE body_name IN ('trivia_started_playing', 'bingo_started_playing', 'candyrush_started_playing')
AND app_version IS NOT NULL AND app_version NOT LIKE '%debug%'
GROUP BY 1,2) b
ON a.date = b.date AND a.landpagetype = b.landpagetype 
...
FROM `processed_db.ua_landing_page_comparision_v4` 
...
FROM (
SELECT *
FROM `processed_db.ua_landing_page_comparision_v4`) a
LEFT JOIN (
SELECT ua_notification_token,device_os 
FROM `master_tables.user_device`
GROUP BY 1,2) b
...
FROM (
SELECT *
FROM `processed_db.CandyKrack_temp_data` 
...
FROM (
SELECT * 
FROM `your-project-id.processed_db.CK_campaign_set2_data`) a
LEFT JOIN (
SELECT * 
FROM `your-project-id.processed_db.CK_campaign_set1_20181127_data`) b
ON a.device_channel = b.device_channel
...
FROM (
SELECT date,device_channel,app_version
FROM `processed_db.ua_landing_page_comparision_v5` 
WHERE type = 'FIRST_OPEN'
AND app_version IS NOT NULL 
AND app_version NOT LIKE '%debug%'
AND app_version NOT LIKE '%unknown_package%'
...
FROM `your-project-id.app_analytics.urban_airship_raw`
WHERE DATE(occurred) = '2018-11-21'
AND type = 'CUSTOM'
...
SELECT * 
FROM `app_analytics.ua_user_opened` 
ORDER BY 1 DESC


SELECT *
FROM `your-project-id.reporting_db.game_players_by_type`  
...
---- Step 1
-- Append the below query data to `your-project-id:reporting_db.ua_app_processed_db_v1`
...
---- Step 2
-- Overwrite the below query data to `your-project-id:reporting_db.ua_retention`
SELECT * FROM (
SELECT Date,(D1/D0) as D1,(D7/D0) as D7,(D14/D0) as D14,(D30/D0) as D30
FROM (
SELECT Date,MAX(IF(Retention = 'D0',Users,NULL)) as D0,MAX(IF(Retention = 'D1',Users,NULL)) as D1,MAX(IF(Retention = 'D7',Users,NULL)) as D7,MAX(IF(Retention = 'D14',Users,NULL)) as D14,MAX(IF(Retention = 'D30',Users,NULL)) as D30
...
SELECT * 
FROM `your-project-id.processed_db.CK_campaign_winners_raw_data` 
...
SELECT a.*,b.device_os FROM (
SELECT * 
FROM `your-project-id.processed_db.CK_campaign_winners_raw_data`)a
LEFT JOIN (
SELECT ua_notification_token as device_channel,device_os
FROM `your-project-id.master_tables.user_device` 
GROUP BY 1,2) b
...
SELECT a.* FROM (
SELECT device_channel 
FROM `your-project-id.processed_db.users_inactive_data` 
GROUP BY 1) a
JOIN (
SELECT device_channel
FROM `your-project-id.app_analytics.urban_airship_v2`
...
FROM (
SELECT device_channel
FROM `your-project-id.processed_db.data_for_AARRR`
WHERE type = 'FIRST_OPEN'
...
GROUP BY 1,2,3) a
CROSS JOIN (
SELECT EXTRACT(WEEK(MONDAY) FROM date) AS week FROM `your-project-id.app_analytics.dates_refer`
WHERE date >= '2018-05-01' AND date <= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
GROUP BY 1) b
...
FROM `your-project-id.gaming_service_db.user_game_statistics` AS ugs
JOIN `your-project-id.gaming_service_db.game` AS g
...
FROM `your-project-id.gaming_service_db.user_statistics`
...
FROM `your-project-id.gaming_service_db.game` g 
JOIN `your-project-id.gaming_service_db.user_game_statistics` ugs 
...
FROM (
SELECT device_channel 
FROM `your-project-id.processed_db.ringers_test_set3`
GROUP BY 1) a
JOIN (
...
FROM (
SELECT device_channel 
FROM `your-project-id.processed_db.ringers_test_set1`
GROUP BY 1) a
JOIN (
SELECT device_channel,type
FROM `your-project-id.reporting_db.ua_app_processed_db_v1` 
...
SELECT 
FROM `your-project-id.gaming_service_db.lives_transaction_history`
...
FROM `your-project-id.processed_db.Candy_krack_points_by_each_game`
GROUP BY 1,2,3,4,5) a
JOIN (
SELECT ua_notification_token as device_channel,user_id
FROM `your-project-id.master_tables.user_device`
...
FROM (
SELECT *
FROM `your-project-id.reporting_db.ua_user_sessions` 
...
select date,count(distinct device_channel) from `app_analytics.ua_processed_db_v1`
where body_name = "swooperstar_recordingstarted" 
...
FROM `your-project-id.processed_db.Cumm_user_wallet_balence`
...
FROM `your-project-id.gaming_service_db.swooperstar_shortlisted_videos` 
...
FROM `your-project-id.gaming_service_db.swooperstar_user_activity` 
GROUP BY 1
...
FROM `your-project-id.processed_db.Cum_user_wallet_balence_info`
GROUP BY 1,2
...
from `app_analytics.urban_airship_v2` where date(occurred) >= "2019-03-01"
...
FROM `your-project-id.reporting_db.ua_user_sessions` a
JOIN (
SELECT a.date,a.device_channel
FROM (
SELECT date,device_channel 
...
FROM `your-project-id.gaming_service_db.user_game_statistics` a
...
FROM (
SELECT date,device_channel
FROM `your-project-id.processed_db.ua_processed_db_v1`
...
FROM `your-project-id.app_analytics.urban_airship_raw` videoplayer_playing
WHERE DATE(occurred) = "2019-03-26"
...
-- app_analytics.ua_processed_db_v2
...
##-- app_analytics.ua_processed_db_v3
#v1
...
FROM `your-project-id.app_analytics.ua_processed_db_v3` 
WHERE DATE(occurred) >= "2019-03-01"
--AND device_channel = "xxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
AND body_name IN ("videoplayer_open","videoplayer_exit")
...
-- processed_db.ua_processed_db_v2
...
def bq_insert_ua_p2p_funnel():
    session_insert_query = """
    INSERT INTO `your-project-id.app_analytics.ua_processed_db_v3` (date, Pay2Play_App_DAU, Pay2Play_GameLisitingUsers, Pay2Play_SuccesfulRegistrations, Pay2Play_GamePlayers) 
    SELECT a.date,COUNT(DISTINCT a.device_channel) as Pay2Play_App_DAU,COUNT(DISTINCT b.device_channel) as Pay2Play_GameLisitingUsers,COUNT(DISTINCT c.device_channel) as Pay2Play_SuccesfulRegistrations,COUNT(DISTINCT d.device_channel) as Pay2Play_GamePlayers
FROM (
SELECT DATE(occurred) as date,device_channel
FROM `your-project-id.app_analytics.ua_processed_db_v3` 
...
def bq_insert_ua_p2p_contest_players():
    session_insert_query = """
    INSERT INTO `your-project-id.app_analytics.ua_processed_db_v3` (date, contest_type, contest_players, contest_plays) 
    SELECT DATE(occurred) as date,contest_type,COUNT(DISTINCT device_channel) as contest_players,COUNT(DISTINCT contest_play_id) as contest_plays
FROM `your-project-id.app_analytics.ua_processed_db_v3` 
WHERE DATE(occurred) = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) --DATE(occurred) >= '2019-03-30' AND DATE(occurred) < CURRENT_DATE()
AND body_name  = 'pay2playgamepage_startedplaying'
...
def bq_insert_ua_p2p_contest_time():
    session_insert_query = """
    INSERT INTO `your-project-id.app_analytics.ua_processed_db_v3` (date, contest_type, time_spent_hrs_in_p2p_games) 
    SELECT date,contest_type,ROUND(SUM(seconds)/3600,2) as time_spent_hrs_in_p2p_games
FROM (
SELECT *,TIMESTAMP_DIFF(occurred, prev_occurred, MILLISECOND)/1000 as seconds
FROM (
...
FROM `your-project-id.processed_db.april_fools_day_campaign_resposnses`
GROUP BY 1,2) a
LEFT JOIN (
...
FROM `your-project-id.gaming_service_db.campaign_user_progress`
...
FROM `your-project-id.master_tables.user`) b
...
FROM `your-project-id.master_tables.user_device`) c
...
LEFT JOIN (
SELECT date,device_channel
FROM `your-project-id.reporting_db.ua_app_processed_db_v1` --Daily_Referral_Retention_Data
WHERE type IN ('OPEN') 
...
FROM `your-project-id.premium_games_db.user_contest_winnings`
...
FROM `your-project-id.premium_games_db.contest`
GROUP BY 1,2,3),
total_info AS (
...
FROM `your-project-id.premium_games_db.user_contest_registration` 
GROUP BY 1) a
JOIN (
...
SELECT date,device_channel
FROM `your-project-id.premium_games_db.ua_pro_game_processed_db_v1` 
...
LEFT JOIN (
SELECT date,device_channel
FROM `your-project-id.premium_games_db.ua_pro_app_processed_db_v1`
WHERE type IN ('OPEN')
AND date >= "2019-05-07"
GROUP BY 1,2) b
ON a.device_channel = b.device_channel
...
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=bingo --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=candy_rush --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=candy_rush_round_detail --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=candy_rush_score_bucket --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=candy_rush_user_score --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=feedback --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=feedback_items --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=game_country --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=game_currency --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=game_signal --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=lives_transaction_history --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=swooperstar_user_activity --default-env delete=yes
#/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=trivia_question_bank --default-env delete=yes
#/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=trivia_question_set --default-env delete=yes
#/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=trivia_user_game_activity --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=user_game_statistics --default-env update_column=updated_at
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=user_lives --default-env delete=yes
/var/lib/jenkins/.local/bin/bonobo run /var/lib/jenkins/swoo-dwh/src/master_tables/master_tables_load.py --default-env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.backend_env_gaming_service_db --default-env table=user_statistics --default-env delete=yes
/usr/bin/python3 /var/lib/jenkins/swoo-dwh/src/master_tables/backend-table-bq-load.py --env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.env-gaming --env CFG_FILE=/var/lib/jenkins/swoo-dwh/src/master_tables/swooperstar_shortlisted_videos.json
/usr/bin/python3 /var/lib/jenkins/swoo-dwh/src/master_tables/backend-table-bq-load.py --env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.env-gaming --env CFG_FILE=/var/lib/jenkins/swoo-dwh/src/master_tables/swooperstar_game.json
/usr/bin/python3 /var/lib/jenkins/swoo-dwh/src/master_tables/backend-table-bq-load.py --env-file /var/lib/jenkins/swoo-dwh/src/master_tables/.env-gaming --env CFG_FILE=/var/lib/jenkins/swoo-dwh/src/master_tables/campaign_user_progress.json




SELECT date(Date_IST) as date,CONCAT(SUBSTR(Time,0,LENGTH(Time)-6),SUBSTR(Time,9,LENGTH(Time))) as Time,game_type_id,game_id,users FROM (SELECT Date_IST,FORMAT_DATETIME('%r',Date_IST) AS Time,B.game_type_id,A.game_id,COUNT(DISTINCT B.user_id) AS users FROM ((SELECT DATETIME(start_time, 'Asia/Kolkata') AS Date_IST,game_id FROM `gaming_service_db.game` WHERE DATE(start_time) = DATE_SUB(CURRENT_DATE(), INTERVAL 2 DAY)) A LEFT JOIN (SELECT user_id,game_id,game_type_id FROM `gaming_service_db.user_game_statistics`) B ON A.game_id = B.game_id) GROUP BY 1,2,3,4 HAVING users > 0 ORDER BY 1)
...
FROM `your-project-id.premium_games_db.ripple_transaction_details`)
WHERE rank = 1 
--AND user_id = 7820788
...
# q1="cqlsh xxx.xxx.xxx.xxx -u db_user -pxxxxxxxxxxxxxxxx -e'select * from user_game_play_detail where created_at >= "
...
cqlsh xxx.xxx.xxx.xxx -u db_user -pxxxxxxxxxxxxxxxx

cqlsh xxx.xxx.xxx.xxx -u db_user -pxxxxxxxxxxxxxxxx -f commands_test.list

sudo docker run -it cassandra /usr/bin/cqlsh xxx.xxx.xxx.xxx -u db_user -pxxxxxxxxxxxxxxxx

cqlsh xxx.xxx.xxx.xxx -u root -pgameqweQWE1!



sudo docker run -it --mount src=/path/to/data,target=/test_container,type=bind cassandra /usr/bin/cqlsh xxx.xxx.xxx.xxx -u db_user -pxxxxxxxxxxxxxxxx
